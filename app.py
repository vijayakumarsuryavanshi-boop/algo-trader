        ...
        with st.container():
            ...
            if auth_mode == "👆 Quick Auth":
                    ...
                    if is_user_blocked(USER_ID):
                        ...
                        if creds and (creds.get("client_id") or creds.get("zerodha_api") or creds.get("coindcx_api") or creds.get("delta_api") or creds.get("fyers_client_id") or creds.get("upstox_api_key") or creds.get("fivepaisa_client_id") or creds.get("binance_api_key") or creds.get("stoxkart_api_key") or creds.get("dhan_client_id") or creds.get("shoonya_user") or creds.get("icici_api_key") or creds.get("kotak_api_key") or creds.get("indmoney_api_key") or creds.get("groww_api_key")):
                            temp_bot = SniperBot(
                                ...
                                mt5_acc=creds.get("mt5_acc", ""), mt5_pass=creds.get("mt5_pass", ""), mt5_server=creds.get("mt5_server", ""), mt5_api_url=creds.get("mt5_api_url", ""),
                                zerodha_api=creds.get("zerodha_api", ""), zerodha_secret=creds.get("zerodha_secret", ""), request_token=creds.get("request_token", ""),
                                ...
                                is_mock=False, tg_bot_token=creds.get("tg_bot_token", ""), tg_allowed_users=creds.get("tg_allowed_users", ""),
                                fyers_client_id=creds.get("fyers_client_id", ""), fyers_secret=creds.get("fyers_secret", ""), fyers_token=creds.get("fyers_token", ""),
                                upstox_api_key=creds.get("upstox_api_key", ""), upstox_api_secret=creds.get("upstox_api_secret", ""), upstox_access_token=creds.get("upstox_access_token", ""),
                                fivepaisa_client_id=creds.get("fivepaisa_client_id", ""), fivepaisa_secret=creds.get("fivepaisa_secret", ""), fivepaisa_access_token=creds.get("fivepaisa_access_token", ""),
                                binance_api_key=creds.get("binance_api_key", ""), binance_api_secret=creds.get("binance_api_secret", ""), binance_testnet=creds.get("binance_testnet", False),
                                stoxkart_api_key=creds.get("stoxkart_api_key", ""), stoxkart_secret=creds.get("stoxkart_secret", ""),
                                dhan_client_id=creds.get("dhan_client_id", ""), dhan_access_token=creds.get("dhan_access_token", ""),
                                shoonya_user=creds.get("shoonya_user", ""), shoonya_password=creds.get("shoonya_password", ""), shoonya_totp=creds.get("shoonya_totp", ""),
                                icici_api_key=creds.get("icici_api_key", ""), icici_secret=creds.get("icici_secret", ""), icici_consumer_key=creds.get("icici_consumer_key", ""),
                                email_smtp_server=creds.get("email_smtp_server", ""), email_port=creds.get("email_port", 587),
                            ...
                            temp_bot.settings["indmoney_api_key"] = creds.get("indmoney_api_key", "")
                            temp_bot.settings["indmoney_secret"] = creds.get("indmoney_secret", "")
                            temp_bot.settings["groww_api_key"] = creds.get("groww_api_key", "")
                            temp_bot.settings["groww_secret"] = creds.get("groww_secret", "")
                ...
                if is_user_blocked(USER_ID):
                    ...
                    with st.container(border=True):
                        col_img, col_t = st.columns([1, 6])
                        with col_img: st.image("https://www.google.com/s2/favicons?domain=icicidirect.com&sz=128", width=40)
                        with col_t: use_icici = st.toggle("ICICI Direct", value=bool(creds.get("icici_api_key")))
                        if use_icici:
                            ICICI_API_KEY = st.text_input("ICICI API Key", value=creds.get("icici_api_key", ""))
                            ICICI_SECRET = st.text_input("ICICI Secret", type="password", value=creds.get("icici_secret", ""))
                            ICICI_CONSUMER_KEY = st.text_input("ICICI Consumer Key", value=creds.get("icici_consumer_key", ""))
                        else:
                            ICICI_API_KEY, ICICI_SECRET, ICICI_CONSUMER_KEY = "", "", ""
                    ...
                    with st.container(border=True):
                        col_img, col_t = st.columns([1, 6])
                        with col_img: st.image("https://www.google.com/s2/favicons?domain=kotak.com&sz=128", width=40)
                        with col_t: use_kotak = st.toggle("Kotak Neo", value=bool(creds.get("kotak_api_key")))
                        if use_kotak:
                            KOTAK_API_KEY = st.text_input("Kotak API Key", value=creds.get("kotak_api_key", ""))
                            KOTAK_SECRET = st.text_input("Kotak Secret", type="password", value=creds.get("kotak_secret", ""))
                        else:
                    ...
                    with st.container(border=True):
                        col_img, col_t = st.columns([1, 6])
                        with col_img: st.image("https://www.google.com/s2/favicons?domain=indmoney.com&sz=128", width=40)
                        with col_t: use_indmoney = st.toggle("IndMoney", value=bool(creds.get("indmoney_api_key")))
                        if use_indmoney:
                            INDMONEY_API_KEY = st.text_input("IndMoney API Key", value=creds.get("indmoney_api_key", ""))
                            INDMONEY_SECRET = st.text_input("IndMoney Secret", type="password", value=creds.get("indmoney_secret", ""))
                        else:
                    ...
                    with st.container(border=True):
                        col_img, col_t = st.columns([1, 6])
                        with col_img: st.image("https://www.google.com/s2/favicons?domain=groww.in&sz=128", width=40)
                        with col_t: use_groww = st.toggle("Groww", value=bool(creds.get("groww_api_key")))
                        if use_groww:
                            GROWW_API_KEY = st.text_input("Groww API Key", value=creds.get("groww_api_key", ""))
                            GROWW_SECRET = st.text_input("Groww Secret", type="password", value=creds.get("groww_secret", ""))
                        else:
                    ...
                    with st.expander("👥 User Role"):
                        ROLE = st.selectbox("Role", ["trader", "admin"], index=0 if creds.get("role", "trader") == "trader" else 1)
                    ...
                    if st.button("CONNECT MARKETS 🚀", type="primary", use_container_width=True, on_click=lambda: play_sound_now("click")):
                        if not USER_ID:
                            ...
                            temp_bot = SniperBot(
                                api_key=ANGEL_API if use_angel else "", client_id=CLIENT_ID if use_angel else "", pwd=PIN if use_angel else "", totp_secret=TOTP if use_angel else "",
                                tg_token=TG_TOKEN, tg_chat=TG_CHAT, wa_phone=WA_PHONE, wa_api=WA_API,
                                mt5_acc=MT5_ACC if use_mt5 else "", mt5_pass=MT5_PASS if use_mt5 else "", mt5_server=MT5_SERVER if use_mt5 else "", mt5_api_url=MT5_API_URL if use_mt5 else "",
                                zerodha_api=Z_API if use_zerodha else "", zerodha_secret=Z_SEC if use_zerodha else "", request_token=Z_REQ if use_zerodha else "",
                                ...
                                is_mock=False, tg_bot_token=TG_BOT_TOKEN, tg_allowed_users=TG_ALLOWED_USERS,
                                ...
                                icici_api_key=ICICI_API_KEY if use_icici else "", icici_secret=ICICI_SECRET if use_icici else "", icici_consumer_key=ICICI_CONSUMER_KEY if use_icici else "",
                            ...
                            # Set Kotak, IndMoney, Groww credentials in settings (the bot will connect them)
                            temp_bot.settings["kotak_api_key"] = KOTAK_API_KEY if use_kotak else ""
                            temp_bot.settings["kotak_secret"] = KOTAK_SECRET if use_kotak else ""
                            temp_bot.settings["indmoney_api_key"] = INDMONEY_API_KEY if use_indmoney else ""
                            temp_bot.settings["indmoney_secret"] = INDMONEY_SECRET if use_indmoney else ""
                            temp_bot.settings["groww_api_key"] = GROWW_API_KEY if use_groww else ""
                            temp_bot.settings["groww_secret"] = GROWW_SECRET if use_groww else ""
                            ...
                            with st.spinner("Authenticating Secure Connections..."):
                                if temp_bot.login():
                                    temp_bot.state["daily_pnl"] = temp_bot.load_daily_pnl()
                                    if SAVE_CREDS:
                                        # Also save the new credentials to the database (include Kotak, IndMoney, Groww)
                                        save_creds(USER_ID, ANGEL_API, CLIENT_ID, PIN, TOTP, TG_TOKEN, TG_CHAT, WA_PHONE, WA_API, MT5_ACC, MT5_PASS, MT5_SERVER, MT5_API_URL, Z_API, Z_SEC, DCX_API, DCX_SEC, DELTA_API, DELTA_SEC, TG_BOT_TOKEN, TG_ALLOWED_USERS, FYERS_CLIENT_ID, FYERS_SECRET, FYERS_TOKEN, UPSTOX_API_KEY, UPSTOX_API_SECRET, UPSTOX_ACCESS_TOKEN, FIVEPAISA_CLIENT_ID, FIVEPAISA_SECRET, FIVEPAISA_ACCESS_TOKEN, BINANCE_API_KEY, BINANCE_API_SECRET, BINANCE_TESTNET, STOXKART_API_KEY, STOXKART_SECRET, DHAN_CLIENT_ID, DHAN_ACCESS_TOKEN, SHOONYA_USER, SHOONYA_PASSWORD, SHOONYA_TOTP, ICICI_API_KEY, ICICI_SECRET, ICICI_CONSUMER_KEY, EMAIL_SMTP_SERVER, EMAIL_PORT, EMAIL_USERNAME, EMAIL_PASSWORD, EMAIL_RECIPIENTS, FCM_SERVER_KEY, PUSH_ENABLED, ROLE, kotak_api_key=KOTAK_API_KEY if use_kotak else "", kotak_secret=KOTAK_SECRET if use_kotak else "", indmoney_api_key=INDMONEY_API_KEY if use_indmoney else "", indmoney_secret=INDMONEY_SECRET if use_indmoney else "", groww_api_key=GROWW_API_KEY if use_groww else "", groww_secret=GROWW_SECRET if use_groww else "")
                                    st.session_state.bot = temp_bot
                                    st.session_state.audio_enabled = True
                                    temp_bot.state["sound_queue"].append("login")
                                    st.session_state.user_id = USER_ID
