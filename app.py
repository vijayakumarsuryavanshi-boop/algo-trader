...
class SniperBot:
    def __init__(self, api_key, client_id, pwd, totp_secret):
        ...
        self.state = {
            "is_running": False,
            "order_in_flight": False, # Ghost Kill lock
            "active_trade": None,
            "logs": [],
            "trade_history": [],
            "current_trend": "WAIT",
    ...
    def emergency_kill(self):
        ...
        if trade:
            if not self.settings.get("paper_mode", True):
                self.place_real_order(trade['symbol'], trade['token'], trade['qty'], "SELL", trade['exch'])
            
            self.log(f"🚨 KILL SWITCH DEPLOYED: Force closed {trade['symbol']}")
    ...
    def trading_loop(self):
        ...
        while self.state["is_running"]:
                        ...
                        if df_candles is not None and not df_candles.empty:
                            ...
                            if self.state["active_trade"] is None and signal in ["BUY_CE", "BUY_PE"] and not self.state["order_in_flight"]:
                                if current_time < cutoff_time:
                                    self.state["order_in_flight"] = True # Lock Thread
                                    try:
                                        ...
                                        self.state["order_in_flight"] = False # Release Lock
                            ...
                            # --- EXIT LOGIC ---
                                ...
                                if ltp:
                                    ...
                                    if current_time >= cutoff_time or ltp >= trade['tgt'] or ltp <= trade['sl']:
                                        if not paper: self.place_real_order(trade['symbol'], trade['token'], trade['qty'], "SELL", trade['exch'])
            ...
            time.sleep(3) # Thread rests for 3 seconds before next cycle
        ...
        if st.button("Connect to Exchange", type="primary"):
            ...
            with st.spinner("Authenticating..."):
                if temp_bot.login():
                    ...
                    temp_bot.log("System Authenticated Successfully")
                    st.rerun()
...
if not st.session_state.bot:
    ...
    st.warning("Please connect using the sidebar to begin.")
    st.stop()

bot = st.session_state.bot
# Update background thread settings from UI
bot.settings = {
    "index": INDEX, "timeframe": TIMEFRAME, "vol_mult": VOL_MULT, "lots": LOTS, 
    "max_capital": MAX_CAPITAL, "sl_pts": SL_PTS, "tgt_pts": TGT_PTS, "paper_mode": PAPER
}
...
with tab1:
    ...
    if bot.state["is_running"]:
        st.success("🚀 Bot is running in the background. You can minimize this window.")
        
        # Dashboard UI
        st.divider()
        m1, m2, m3, m4 = st.columns(4)
        m1.metric(f"Live {INDEX}", bot.state["spot"])
        ...
        if bot.state["active_trade"]:
            ...
            st.info(f"📈 Open: **{t['symbol']}** | Entry: **{t['entry']}** | LTP: **{ltp}** | PnL: {indicator} **₹{round(pnl, 2)}**")
            
            if st.button("Close Manually"):
                if not PAPER: bot.place_real_order(t['symbol'], t['token'], t['qty'], "SELL", t['exch'])
