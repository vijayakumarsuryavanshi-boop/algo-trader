...
if bot.state.get("latest_data") is not None and not bot.state["latest_data"].empty:
    ...
    if len(df_today) >= 20:
        ...
        golden_low  = swing_high - (diff * 0.65)
    else:
        golden_high = golden_low = 0.0

    # Beautiful Animated CSS
    st.markdown("""
    <style>
    .key-levels-grid {
        display: grid;
